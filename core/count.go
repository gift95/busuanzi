...
func Count(ctx context.Context, host string, path string, userIdentity string) Counts {
	...
	_redis.SAdd(ctx, rk.SiteUvKey, userIdentity)
	_redis.SAdd(ctx, rk.PageUvKey, userIdentity)

	// count siteUv and pageUv
	siteUv, _ := _redis.SCard(ctx, rk.SiteUvKey).Result()
	pageUv, _ := _redis.SCard(ctx, rk.PageUvKey).Result()
